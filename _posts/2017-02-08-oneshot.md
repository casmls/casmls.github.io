...
This week, by the lead of Yuanjun Gao, we discussed two papers,  "Human-level concept learning through probabilistic program induction." by Lake et al [1] and "One-Shot Generalization in Deep Generative Models." by Danilo J. Rezende et al [2]. Two papers aim towards human's ability to learn from small number of examples. The first paper introduces Bayesian Program Learning framework (BPL), a probabilistic model which allows such learning ability and the second paper implements the idea in deep generative model.
...
1. For each part, add a motor variation (as if a hand written character looks slightly different every time).
...
The three abilities of human described in the beginning is tested.
...
The paper also tried the same task with BPL but missing crucial step. Lesion on "Learning-to-learn" (no type generative process) caused 11% error rate. Lesion on "Compositionality" (each character is learned using only a single stroke) caused 14%. These two results indicate how fundamental those two ideas are in the BPL.
...
The BPL's ability to understand structure in visual concept is still very limited. It lacks the knowledge of parallel lines, symmetry, optional elements such as cross bars, and many others. It also lacks human's ability to use learned concepts for others such as planning, communication, and conceptual combination [1].  However, as humans do, these are something learned from a long term education. One-shot learning and generalization of such seemed not an easy task. The paper also mentions the hope for neural representations of concepts and the development of more neurally grounded learning models, which leads to the next paper.
...
The paper incorporates the deep generative process to model human's one-shot generalization ability. It shows that the process can generate new examples or new types when unseen examples are provided. Compared to the BPL model by Lake [1], which uses naturally occurring causal structures of objects, this method is applicable to any methods. However, there are few limitations, which can lead to future study. It still needs reasonable amount of data to avoid overfitting. While it is applicable to any images (by not using specific structures), it cannot parse structures into smaller parts.
...
[4] Gregor, K., Danihelka, I., Graves, A., Rezende, D. J., and Wierstra, D. DRAW: A recurrent neural network for im- age generation. In ICML, 2015.
...
[6] Eslami, S. M., Heess, N., Weber, T., Tassa, Y., Kavukcuoglu, K., and Hinton, G. E. Attend, Infer, Re- peat: Fast scene understanding with generative models. arXiv preprint arXiv:1603.08575, 2016.