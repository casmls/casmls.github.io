...
2.  Calculating the free energy of the system using the replica trick 
    A physical system in the microcanonical ensemble minimizes $$E$$. A physical system in the canonical ensemble (held at constant temperature $$\frac{1}{\beta}$$) minimizes the free     energy
...
3.  Taking the $$\beta\rightarrow\infty$$ limit.
    ...
    \left\langle Z^{n}\right\rangle = \left\langle \int \prod_{a=1}^{n} \mathrm{d}u^{a} e^{-\sum_a \beta E(u^{a})} \right\rangle 
     = \int \left\langle \int \prod_{a,b=1}^{n} \mathrm{d}Q_{ab} \mathrm{d}\hat{Q}_{ab} e^{-Nf(Q,\hat{Q})} \right\rangle 
    $$
...
can be replaced by $$4$$ variables in the form $$Q=\text{diag}(\Delta)+ \mathbb{1}(Q_{0})$$.
This is a mean field assumption on the interaction between the replicas:
$$r_{ij}x_{i}x_{j}\approx\sum r_{1}x_{i}^{2}+r_{0}x_{i}$$.
The results are:

$$ 
q_{d}=\frac{\left\langle M'_{\lambda_{\rho}}[\rho](\varepsilon_{q_{s}})^{2}\right\rangle_{\varepsilon_{q_{s}}}}{\alpha\left\langle M''_{\lambda_{\rho}}[\rho](\varepsilon_{q_{s}})\right\rangle _{\varepsilon_{q_{s}}}^{2}} 
$$
