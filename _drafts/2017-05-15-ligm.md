...
Again, from a physical perspective, if we have a Lagrangian, we would like to find a path $$\{ X_t : t \in \mathbb{T}\}$$ which minimizes the _action_

$$ \mathcal{J}[X] = \int_{\mathbb{T}} \mathcal{L}(X_t, \dot{X}_t, t ) \, dt $$

(where over-dot's denote differentiation with respect to time) which is a functional on paths. 

## The Euler-Lagrange equation

We can then use the _calculus of variations_ in order to find a curve which minimizes this functional. 
