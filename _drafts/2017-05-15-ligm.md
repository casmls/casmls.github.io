...
_Class-probability matching_ - This boils down to using a classifier which can distingush between observed data and that generated from the model. Although GANs come under this approach, the authors give a few references to other methods which also fall under this umbrella:

(refs)

To describe the general approach, let $$Y$$ denote a random variable which assigns label $$Y = 1$$ to samples from the true data distribution, and $$Y = 0$$ to those from the model distribution, so we can write $$p*(x) = p(x|Y = 1)$$ and $$q_{\theta}(x) = p(x|Y = 0)$$. Writing $$p(y = 1) = \pi$$, it therefore follows by Bayes' formula that
...
Here the choice of $$s$$ is very important to the performance of the procedure, as ideally we would like all the moments of the distributions to be matched.
